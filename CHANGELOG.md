...
- État intégré courant : **Web 1.8.16**, **Serveur / installateur 1.1.2**, **BDD 0.91.2**, **Agent 1.0.1**.
...
- Le Web 1.8.16 et le Serveur 1.1.2 introduisent les types critiques `CB` / `CH` : mêmes couleurs que B/H, indicateur danger réservé aux critiques, compatibilité des APIs/acquittements/statistiques et conservation du type de déclenchement pendant toute l'alarme.
- La BDD 0.91.2 élargit à deux caractères `t_alarme.Type`, `t_alarme_histo.Type` et `t_alarme_message.Type`, ajoute les messages critiques 20/21 et conserve le trigger GSO de 0.91.1 inchangé.
