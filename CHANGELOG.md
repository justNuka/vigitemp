...
- État intégré courant : **Web 1.9.2**, **Serveur / installateur 1.2.0**, **BDD 0.92.0**, **Agent 1.0.1**.
...
- Le Web 1.9.0 et le Serveur 1.2.0 introduisent les types critiques `CB` / `CH` : mêmes couleurs que B/H, indicateur danger réservé aux critiques, compatibilité des APIs/acquittements/statistiques et conservation du type de déclenchement pendant toute l'alarme.
- Le Web 1.9.1 précise dans Ajustage et Étalonnage que la cadence des lectures GSP est une cible : lorsqu'un module dessert aussi des sondes en Surveillance, celle-ci reste prioritaire et peut légèrement décaler l'interrogation de métrologie.
- Le Web 1.9.2 fiabilise la préparation des packages avec pnpm : politique `allowBuilds` déclarative, suppression de l'approbation interactive et installation stricte via `--frozen-lockfile`.
- La BDD 0.92.0 élargit à deux caractères `t_alarme.Type`, `t_alarme_histo.Type` et `t_alarme_message.Type`, ajoute les messages critiques 20/21 et réactive le déclenchement critique GSO en `CB` / `CH` sur MySQL et SQL Server.
