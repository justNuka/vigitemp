-- VigiSensys DB 0.91.2 - MySQL 8.x
-- Baseline attendue : 0.91.1
USE `vigi_main`;

ALTER TABLE `t_alarme` MODIFY COLUMN `Type` VARCHAR(2) DEFAULT NULL;
ALTER TABLE `t_alarme_histo` MODIFY COLUMN `Type` VARCHAR(2) DEFAULT NULL;
ALTER TABLE `t_alarme_message` MODIFY COLUMN `Type` VARCHAR(2) DEFAULT NULL;

INSERT INTO `t_alarme_message` (`Id_Alarme_Message`,`Code_Alarme_Message`,`Type`,`Texte_Message`) VALUES
...
ON DUPLICATE KEY UPDATE `Code_Alarme_Message`=VALUES(`Code_Alarme_Message`),`Type`=VALUES(`Type`),`Texte_Message`=VALUES(`Texte_Message`);
...
ON DUPLICATE KEY UPDATE `Valeur`=VALUES(`Valeur`),`Commentaire`=VALUES(`Commentaire`);
