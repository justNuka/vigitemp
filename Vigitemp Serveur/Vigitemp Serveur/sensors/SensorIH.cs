...
namespace Vigitemp_Serveur.sensors
{
    class SensorIH : Sensor
    {
        ...
        public SensorIH(ThreadServeur p_ths, string p_comPort, string p_sondeSerialNumber, string p_sondeAdresse) : base(p_ths, p_comPort, p_sondeSerialNumber, p_sondeAdresse)
        {
            ...
            // La trame IH est hybride : en-tete ASCII + 2 octets de mesure binaires.
                            ...
                            SerialDataReceivedEventArgs e)
        {
            try
            {
                ...
                VigitempServeur.Log($"[SONDE][RX] type=IH serial={m_sondeSerialNumber} high=0x{poidsFort:X2} low=0x{poidsFaible:X2} resistance={tmp_resistance}");
                if (int.Parse(tmp_resistance) > -2048 && int.Parse(tmp_resistance) < 2048)
                {
                    ...
                    compareMeasuresAndLimits(correctedValue, "%HR");
                    ths.GetDatabase().AddMesure(m_sondeSerialNumber, correctedValue, "%HR", ToInvariantRaw(rawValue));
                    VigitempServeur.Log($"[SONDE][DONE] type=IH serial={m_sondeSerialNumber} port={m_comPort} status=success value={correctedValue} unit=%HR raw={ToInvariantRaw(rawValue)}");
                }
                else
                {
