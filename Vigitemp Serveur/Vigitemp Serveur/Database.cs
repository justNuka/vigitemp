    ...
    class Database : IDatabaseProvider
    {
        ...
        public (List<float>, bool notificationActive, DateTime Date_Heure_Reactivation_Alarme) getConsignesLieux(int p_idLieu)
        {
            lock (_lock)
            {
                ...
                cmd_vigitemp.CommandText = "select " +
                                            "IFNULL(Tolerance_Surveillance_Sup, Consigne_Sup) as Consigne_Sup, " +
                                            "IFNULL(Tolerance_Surveillance_Inf, Consigne_Inf) as Consigne_Inf, " +
        ...
        private LieuAlarmSettings ReadLieuAlarmSettingsV2(int idLieu)
        {
            ...
            cmd.CommandText =
                "SELECT " +
                "Id_Lieu, " +
                "IFNULL(Tolerance_Surveillance_Inf, Consigne_Inf) as Consigne_Inf, Est_Consigne_Inf_Active, Retard_Alarme_Bas, Consigne_Inf_Pre_Alarme, Est_Consigne_Inf_Pre_Alarme_Active, " +
                "IFNULL(Tolerance_Surveillance_Sup, Consigne_Sup) as Consigne_Sup, Est_Consigne_Sup_Active, Retard_Alarme_Haut, Consigne_Sup_Pre_Alarme, Est_Consigne_Sup_Pre_Alarme_Active, " +
                "Retard_Non_Reponse " +
        ...
        public bool hasActiveAcknowledgedAlarm(int idLieu, string type)
        {
            lock (_lock)
            {
                ...
                {
                    CloseConnexion();
                    VigitempServeur.Log("(hasActiveAcknowledgedAlarm) SQL Erreur: " + ex.Message);
                    return false;
                }
            }
        }
