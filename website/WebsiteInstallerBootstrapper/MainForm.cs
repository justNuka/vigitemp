...
public sealed class MainForm : Form
{
    ...
    private readonly CheckBox autoConfigureFirewall;
    ...
    public MainForm()
    {
        ...
        Controls.AddRange(new Control[]
        {
            ...
            new Label { Width = 900, Height = 44, Location = new Point(24, 56), ForeColor = Color.FromArgb(226, 232, 240), BackColor = TextPrimary, Text = "Renseignez les parametres d'installation, relisez le resume, puis lancez l'installation. Vous pouvez revenir en arriere avant l'execution." },
        ...
        var cspPlaceholder = string.IsNullOrWhiteSpace(localIpv4) ? "http://<ip-machine>:8000,http://localhost:8000" : $"http://{localIpv4}:8000,http://localhost:8000";

        installDir = T(_s.InstallDir, placeholder: @"C:\ProgramData\VigiSensys\website"); serviceName = T(_s.ServiceName, placeholder: "VigiSensysWeb"); port = T(_s.Port, placeholder: "3000"); websiteBaseUrl = T(_s.WebsiteBaseUrl, placeholder: websiteUrlPlaceholder); appBaseUrl = T(_s.AppBaseUrl, placeholder: websiteUrlPlaceholder);
        ...
        dbHost = T(_s.DbHost, placeholder: genericIpPlaceholder); dbPort = T(_s.DbPort, placeholder: "3306"); dbUser = T(_s.DbUser, placeholder: "Compte SQL dedie (pas root)"); dbPassword = T(_s.DbPassword, true, placeholder: "Mot de passe BDD"); dbMain = T(_s.DbMain, placeholder: "vigi_main"); dbMeasure = T(_s.DbMeasure, placeholder: "vigi_mesures"); dbChat = T(_s.DbChat, placeholder: "vigi_chat"); cacheTtl = T(_s.CacheTtl, placeholder: "30");
        ...
        detectFilesButton = SecondaryButton("Detecter les fichiers", 170); detectFilesButton.Click += (_, _) => DetectSecurityFiles();
        agentPort = T(_s.AgentPort, placeholder: "8000"); agentTimeoutMs = T(_s.AgentTimeoutMs, placeholder: "1500"); agentActiveWindowMinutes = T(_s.AgentActiveWindowMinutes, placeholder: "15"); hotlineServerHost = T(_s.HotlineServerHost, placeholder: genericIpPlaceholder); hotlineServerPort = T(_s.HotlineServerPort, placeholder: "5310"); hotlineServerTimeoutMs = T(_s.HotlineServerTimeoutMs, placeholder: "10000"); hotlineAccessTokenTtl = T(_s.HotlineAccessTokenTtl, placeholder: "15"); hotlineRefreshTokenTtl = T(_s.HotlineRefreshTokenTtl, placeholder: "120"); allowedDevOrigins = T(_s.AllowedDevOrigins, placeholder: ""); cspConnectSrc = T(_s.CspConnectSrc, placeholder: cspPlaceholder); dispatchSecret = T(_s.DispatchSecret, placeholder: "Laisser vide pour reprise automatique"); emailTimezone = T(_s.EmailTimezone, placeholder: "Europe/Paris");
        autoConfigureFirewall = new CheckBox { Dock = DockStyle.Fill, Text = "Crer automatiquement les rgles firewall Windows (web + agent)", Checked = _s.AutoConfigureFirewall, ForeColor = TextPrimary, BackColor = CardBackground };
        ...
        var database = StepPanel(); foreach (var c in new Control[] { Field("Type de BDD", dbProvider), Field("Hote BDD", dbHost), Field("Port BDD", dbPort), Field("Utilisateur BDD", dbUser), Field("Mot de passe BDD", dbPassword), Field("BDD principale", dbMain), Field("BDD mesures", dbMeasure), Field("BDD chat", dbChat), Field("Cache TTL (secondes)", cacheTtl) }) database.Controls.Add(c);
        var security = StepPanel(); foreach (var c in new Control[] { Field("Dossier des logs", logsDir, BrowseFolder(logsDir)), Field("Fichier licence (.vtlic)", licensePath, BrowseFile(licensePath, "Licence (*.vtlic)|*.vtlic|Tous les fichiers (*.*)|*.*")), Field("Clee publique licence (.pem)", publicKeyPath, BrowseFile(publicKeyPath, "PEM (*.pem)|*.pem|Tous les fichiers (*.*)|*.*")), Field("Clee privee agent (.pem)", agentPrivateKeyPath, BrowseFile(agentPrivateKeyPath, "PEM (*.pem)|*.pem|Tous les fichiers (*.*)|*.*")) }) security.Controls.Add(c);
        var advanced = StepPanel(); foreach (var c in new Control[] { Field("Port agent local", agentPort), Field("Timeout agent local (ms)", agentTimeoutMs), Field("Fenetre active agent (minutes)", agentActiveWindowMinutes), Field("Hote serveur hotline", hotlineServerHost), Field("Port serveur hotline", hotlineServerPort), Field("Timeout hotline (ms)", hotlineServerTimeoutMs), Field("TTL access hotline (minutes)", hotlineAccessTokenTtl), Field("TTL refresh hotline (minutes)", hotlineRefreshTokenTtl), Field("Fuseau horaire emails (IANA)", emailTimezone), Field("Origins reverse proxy / Server Actions (CSV, optionnel)", allowedDevOrigins), Field("CSP connect-src supplementaires (CSV, optionnel)", cspConnectSrc), Field("Secret dispatch alarmes (laisser vide pour reprendre/generer)", dispatchSecret), Field("Firewall Windows", autoConfigureFirewall) }) advanced.Controls.Add(c);
        var installPanel = new Panel { Padding = new Padding(20), BackColor = CardBackground };
        installPanel.Controls.Add(_log); installPanel.Controls.Add(_status); installPanel.Controls.Add(_summary); installPanel.Controls.Add(new Label { Text = "Rsum avant installation", Dock = DockStyle.Top, Height = 24, ForeColor = TextPrimary, Font = new Font("Segoe UI Semibold", 10F, FontStyle.Bold) });

        _tabs.TabPages.AddRange(new[] { Page("General", general), Page("Base de donnees", database), Page("Fichiers et securite", security), Page("Parametres avances", advanced), Page("Rsum et installation", installPanel) });
    ...
    private bool Valid(int idx)
    {
        ...
        else if (idx == 1) { if (string.IsNullOrWhiteSpace(_s.DbHost)) m = "L'hte BDD est obligatoire."; else if (string.IsNullOrWhiteSpace(_s.DbPort)) m = "Le port BDD est obligatoire."; else if (string.IsNullOrWhiteSpace(_s.DbUser)) m = "L'utilisateur BDD est obligatoire."; else if (string.IsNullOrWhiteSpace(_s.DbMain) || string.IsNullOrWhiteSpace(_s.DbMeasure) || string.IsNullOrWhiteSpace(_s.DbChat)) m = "Les noms de bases sont obligatoires."; else if (string.IsNullOrWhiteSpace(_s.CacheTtl)) m = "Le cache TTL est obligatoire."; }
        else if (idx == 2) { if (string.IsNullOrWhiteSpace(_s.LogsDir)) m = "Le dossier des logs est obligatoire."; else if (string.IsNullOrWhiteSpace(_s.LicensePath) || !File.Exists(_s.LicensePath)) m = "Le fichier licence est obligatoire."; else if (string.IsNullOrWhiteSpace(_s.PublicKeyPath) || !File.Exists(_s.PublicKeyPath)) m = "La cl publique licence est obligatoire."; else if (!isPackLicense && (string.IsNullOrWhiteSpace(_s.AgentPrivateKeyPath) || !File.Exists(_s.AgentPrivateKeyPath))) m = "La cl prive agent est obligatoire."; }
        else if (idx == 3) { if (string.IsNullOrWhiteSpace(_s.AgentPort)) m = "Le port agent local est obligatoire."; else if (string.IsNullOrWhiteSpace(_s.AgentTimeoutMs)) m = "Le timeout agent local est obligatoire."; else if (string.IsNullOrWhiteSpace(_s.AgentActiveWindowMinutes)) m = "La fentre active agent est obligatoire."; else if (string.IsNullOrWhiteSpace(_s.HotlineServerHost)) m = "L'hte hotline est obligatoire."; else if (string.IsNullOrWhiteSpace(_s.HotlineServerPort)) m = "Le port hotline est obligatoire."; else if (string.IsNullOrWhiteSpace(_s.HotlineServerTimeoutMs)) m = "Le timeout hotline est obligatoire."; else if (string.IsNullOrWhiteSpace(_s.HotlineAccessTokenTtl) || string.IsNullOrWhiteSpace(_s.HotlineRefreshTokenTtl)) m = "Les TTL hotline sont obligatoires."; else if (string.IsNullOrWhiteSpace(_s.EmailTimezone)) m = "Le fuseau horaire emails est obligatoire."; }
        if (m != null) { MessageBox.Show(m, "Configuration incomplete", MessageBoxButtons.OK, MessageBoxIcon.Warning); return false; }
        return true;
    }
    ...
    private void Go(int idx)
    {
        ...
        var titles = new[] { "General", "Base de donnees", "Fichiers et securite", "Parametres avances", "Rsum et installation" };
        _step.Text = $"tape {idx + 1} / {titles.Length} - {titles[idx]}";
        _back.Enabled = idx > 0;
        _next.Enabled = idx < 4;
    ...
    private void RefreshSummary()
    {
        ...
        sb.AppendLine("General"); sb.AppendLine($"- Dossier d'installation : {_s.InstallDir}"); sb.AppendLine($"- Service Windows : {_s.ServiceName}"); sb.AppendLine($"- Port HTTP : {_s.Port}"); sb.AppendLine($"- URL site : {_s.WebsiteBaseUrl}"); sb.AppendLine($"- URL applicative : {_s.AppBaseUrl}"); sb.AppendLine();
        sb.AppendLine("Base de donnees"); sb.AppendLine($"- Provider : {_s.DbProvider}"); sb.AppendLine($"- Hote : {_s.DbHost}:{_s.DbPort}"); sb.AppendLine($"- Utilisateur : {_s.DbUser}"); sb.AppendLine($"- BDD principale : {_s.DbMain}"); sb.AppendLine($"- BDD mesures : {_s.DbMeasure}"); sb.AppendLine($"- BDD chat : {_s.DbChat}"); sb.AppendLine();
        sb.AppendLine("Fichiers et securite"); sb.AppendLine($"- Logs : {_s.LogsDir}"); sb.AppendLine($"- Licence : {_s.LicensePath}"); sb.AppendLine($"- Clee publique : {_s.PublicKeyPath}"); sb.AppendLine($"- Clee privee agent : {(isPackLicense ? "non requise (licence Pack)" : _s.AgentPrivateKeyPath)}"); sb.AppendLine();
        sb.AppendLine("Avanc"); sb.AppendLine($"- Agent : {_s.AgentPort} / {_s.AgentTimeoutMs} ms / {_s.AgentActiveWindowMinutes} min"); sb.AppendLine($"- Hotline : {_s.HotlineServerHost}:{_s.HotlineServerPort} / {_s.HotlineServerTimeoutMs} ms"); sb.AppendLine($"- TTL hotline : access {_s.HotlineAccessTokenTtl} min / refresh {_s.HotlineRefreshTokenTtl} min"); sb.AppendLine($"- Fuseau horaire emails : {_s.EmailTimezone}"); sb.AppendLine($"- Origins reverse proxy / Server Actions : {_s.AllowedDevOrigins}"); sb.AppendLine($"- CSP connect-src : {_s.CspConnectSrc}"); sb.AppendLine($"- Firewall Windows : {(_s.AutoConfigureFirewall ? "configuration automatique" : "configuration manuelle")}"); sb.AppendLine($"- Secret dispatch : {(string.IsNullOrWhiteSpace(_s.DispatchSecret) ? "gener / repris automatiquement" : "fourni manuellement")}"); sb.AppendLine("- Better Auth : active (API publique desactivee)");
    ...
    private async System.Threading.Tasks.Task InstallAsync()
    {
        ...
        if (string.IsNullOrWhiteSpace(node)) { MessageBox.Show("Node.js est introuvable dans le PATH. Installez d'abord les prerequis.", "Installation impossible", MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
        ...
        try
        {
            Directory.CreateDirectory(_s.InstallDir); InstallerHelpers.CopyDirectory(src, _s.InstallDir); Directory.CreateDirectory(_s.LogsDir); AppendLog("[OK] Fichiers copies.");
            ...
            var betterAuthSecret = InstallerHelpers.GetOrCreateSharedSecret(src, "better-auth-secret.txt");
            ...
            if (string.Equals(_s.DbProvider, "mysql", StringComparison.OrdinalIgnoreCase) && string.Equals(_s.DbUser, "root", StringComparison.OrdinalIgnoreCase)) throw new InvalidOperationException("Le compte MySQL root n'est pas supporte par cette installation. Creez un compte SQL dedie.");
            ...
            AppendLog($"[OK] Fichiers de securite copies vers ProgramData: {normalizedLicensePath}, {normalizedPublicKeyPath}{(isPackLicense ? string.Empty : ", " + normalizedAgentPrivateKeyPath)}");
  ...
  <env name=""HOSTNAME"" value=""0.0.0.0"" />
            ...
            File.WriteAllText(winswXml, xml); InstallerHelpers.RunProcess(winswExe, "install", _s.InstallDir, AppendLog); InstallerHelpers.RunProcess(winswExe, "start", _s.InstallDir, AppendLog); if (_s.AutoConfigureFirewall) { InstallerHelpers.EnsureFirewallRules("VigiSensys", int.Parse(_s.Port), int.Parse(_s.AgentPort), AppendLog); } var uninstallScriptPath = InstallerHelpers.WriteWebUninstallScript(_s.InstallDir, _s.ServiceName); var displayIconPath = InstallerHelpers.WriteInstalledDisplayIcon(_s.InstallDir, "VigiSensysWeb", Application.ExecutablePath) ?? winswExe; InstallerHelpers.WriteRegistryInfo(_s.InstallDir, string.Empty, _s.ServiceName, displayIconPath, uninstallScriptPath); InstallerHelpers.RemoveInstallerArtifacts(_s.InstallDir); AppendLog("[OK] Artefacts d'installation supprimes du dossier install.");
            SetStatus("Etat : installation termine avec succs"); AppendLog("[OK] Installation terminee."); MessageBox.Show("Installation du site termine. Vrifiez le service Windows si ncessaire.", "Installation terminee", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
