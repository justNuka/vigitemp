...
public sealed class MainForm : Form
{
    ...
    private readonly Button _back = new() { Text = "PrÃ©cÃ©dent", Width = 120, Height = 38, Left = 24, Top = 666, Anchor = AnchorStyles.Left | AnchorStyles.Bottom };
    ...
    private readonly Label _status = new() { Text = "Etat : prÃªt", Dock = DockStyle.Top, Height = 30, Padding = new Padding(0, 8, 0, 0), ForeColor = Accent, Font = new Font("Segoe UI Semibold", 9F, FontStyle.Bold) };
    ...
    public MainForm()
    {
        ...
        Controls.AddRange(new Control[]
        {
            ...
            new Label { Width = 900, Height = 44, Location = new Point(24, 56), ForeColor = Color.FromArgb(226, 232, 240), BackColor = TextPrimary, Text = "Renseignez les paramÃ¨tres d'installation, relisez le rÃ©sumÃ©, puis lancez l'installation. Vous pouvez revenir en arriÃ¨re avant l'exÃ©cution." },
        ...
        dbHost = T(_s.DbHost, placeholder: genericIpPlaceholder); dbPort = T(_s.DbPort, placeholder: "3306"); dbUser = T(_s.DbUser, placeholder: "Compte SQL dédié (pas root)"); dbPassword = T(_s.DbPassword, true, placeholder: "Mot de passe BDD"); dbMain = T(_s.DbMain, placeholder: "vigi_main"); dbMeasure = T(_s.DbMeasure, placeholder: "vigi_mesures"); dbChat = T(_s.DbChat, placeholder: "vigi_chat"); cacheTtl = T(_s.CacheTtl, placeholder: "30");
        ...
        detectFilesButton = SecondaryButton("DÃ©tecter les fichiers", 170); detectFilesButton.Click += (_, _) => DetectSecurityFiles();
        ...
        autoConfigureFirewall = new CheckBox { Dock = DockStyle.Fill, Text = "Créer automatiquement les règles firewall Windows (web + agent)", Checked = _s.AutoConfigureFirewall, ForeColor = TextPrimary, BackColor = CardBackground };
        ...
        var database = StepPanel(); foreach (var c in new Control[] { Field("Type de BDD", dbProvider), Field("HÃ´te BDD", dbHost), Field("Port BDD", dbPort), Field("Utilisateur BDD", dbUser), Field("Mot de passe BDD", dbPassword), Field("BDD principale", dbMain), Field("BDD mesures", dbMeasure), Field("BDD chat", dbChat), Field("Cache TTL (secondes)", cacheTtl) }) database.Controls.Add(c);
        var security = StepPanel(); foreach (var c in new Control[] { Field("Dossier des logs", logsDir, BrowseFolder(logsDir)), Field("Fichier licence (.vtlic)", licensePath, BrowseFile(licensePath, "Licence (*.vtlic)|*.vtlic|Tous les fichiers (*.*)|*.*")), Field("ClÃ© publique licence (.pem)", publicKeyPath, BrowseFile(publicKeyPath, "PEM (*.pem)|*.pem|Tous les fichiers (*.*)|*.*")), Field("ClÃ© privÃ©e agent (.pem)", agentPrivateKeyPath, BrowseFile(agentPrivateKeyPath, "PEM (*.pem)|*.pem|Tous les fichiers (*.*)|*.*")) }) security.Controls.Add(c);
        var advanced = StepPanel(); foreach (var c in new Control[] { Field("Port agent local", agentPort), Field("Timeout agent local (ms)", agentTimeoutMs), Field("FenÃªtre active agent (minutes)", agentActiveWindowMinutes), Field("HÃ´te serveur hotline", hotlineServerHost), Field("Port serveur hotline", hotlineServerPort), Field("Timeout hotline (ms)", hotlineServerTimeoutMs), Field("TTL access hotline (minutes)", hotlineAccessTokenTtl), Field("TTL refresh hotline (minutes)", hotlineRefreshTokenTtl), Field("Fuseau horaire emails (IANA)", emailTimezone), Field("Origins dev autorisÃ©s (CSV, optionnel)", allowedDevOrigins), Field("CSP connect-src supplÃ©mentaires (CSV, optionnel)", cspConnectSrc), Field("Secret dispatch alarmes (laisser vide pour reprendre/gÃ©nÃ©rer)", dispatchSecret), Field("Firewall Windows", autoConfigureFirewall) }) advanced.Controls.Add(c);
        var installPanel = new Panel { Padding = new Padding(20), BackColor = CardBackground };
        installPanel.Controls.Add(_log); installPanel.Controls.Add(_status); installPanel.Controls.Add(_summary); installPanel.Controls.Add(new Label { Text = "RÃ©sumÃ© avant installation", Dock = DockStyle.Top, Height = 24, ForeColor = TextPrimary, Font = new Font("Segoe UI Semibold", 10F, FontStyle.Bold) });

        _tabs.TabPages.AddRange(new[] { Page("GÃ©nÃ©ral", general), Page("Base de donnÃ©es", database), Page("Fichiers et sÃ©curitÃ©", security), Page("ParamÃ¨tres avancÃ©s", advanced), Page("RÃ©sumÃ© et installation", installPanel) });
    ...
    private void Go(int idx)
    {
        ...
        var titles = new[] { "GÃ©nÃ©ral", "Base de donnÃ©es", "Fichiers et sÃ©curitÃ©", "ParamÃ¨tres avancÃ©s", "RÃ©sumÃ© et installation" };
        _step.Text = $"Ã‰tape {idx + 1} / {titles.Length} - {titles[idx]}";
        _back.Enabled = idx > 0;
        _next.Enabled = idx < 4;
    ...
    private async System.Threading.Tasks.Task InstallAsync()
    {
        ...
        if (string.IsNullOrWhiteSpace(node)) { MessageBox.Show("Node.js est introuvable dans le PATH. Installez d'abord les prÃ©requis.", "Installation impossible", MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
        ...
        try
        {
            Directory.CreateDirectory(_s.InstallDir); InstallerHelpers.CopyDirectory(src, _s.InstallDir); Directory.CreateDirectory(_s.LogsDir); AppendLog("[OK] Fichiers copiÃ©s.");
