...
export async function hasUserAvatarColumn() {
  ...
  try {
    const rows = await prisma.$queryRaw<Array<{ cnt: number | bigint | string }>>`
      SELECT COUNT(*) AS cnt
...
export async function getUserAvatarValue(userId: number) {
  ...
  try {
    const rows = await prisma.$queryRawUnsafe<Array<{ Avatar_Utilisateur: string | null }>>(
...
export async function getUserAvatarMap(userIds: number[]) {
  ...
  try {
    const rows = await prisma.$queryRawUnsafe<Array<{ Id_Utilisateur: number; Avatar_Utilisateur: string | null }>>(
