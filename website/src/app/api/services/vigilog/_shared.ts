...
export const VIGILOG_TEMP_USAGE_STATUSES = ["EN_COURS", "TERMINE"] as const
...
export const vigilogTemporaryUsageStartSchema = z.object({
  Id_VigiLog_Configuration: positiveIntegerSchema,
  Id_VigiLog: positiveIntegerSchema.nullable().optional(),
  Numero_Serie_VigiLog: z.string().trim().min(1).max(30),
  Nom_Lieu_Temporaire: z.string().trim().min(1).max(120),
  Commentaire_Demarrage: z.string().trim().max(2000).nullable().optional(),
})

export const vigilogTemporaryUsageStopSchema = z.object({
  Commentaire_Arret: z.string().trim().max(2000).nullable().optional(),
})
...
export function buildVigilogTemporaryUsageReference(userId: number) {
  const now = new Date()
  const datePart = [
    now.getFullYear(),
    String(now.getMonth() + 1).padStart(2, "0"),
  ...
  return `VLGU-${datePart}-${timePart}-${userId}`
}
