...
export const GET = withAuthLogging(async (req: NextRequest, ctx) => {
  try {
    ...
    // L'analyse d'impact porte sur les alarmes DECLENCHEES pendant la plage
    // choisie. Une vieille alarme encore ouverte ne doit donc pas remonter
    // simplement parce que sa date de fin est nulle.
    ...
    const alarms = merged.map((a) => ({
      ...
      Date_Heure_Debut: serializePrismaStoredDbDateTime(a.Date_Heure_Debut) ?? startDate,
      Date_Heure_Fin: serializePrismaStoredDbDateTime(a.Date_Heure_Fin) ?? null,
      Type: a.Type,
    }))
