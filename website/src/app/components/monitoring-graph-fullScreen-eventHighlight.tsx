...
export default function MonitoringGraphFullScreen_EventHighlight(this: any, { idLieu, NomLieu, stateNeedReload, setStateNeedReload, _startDate, _endDate, eventHistory, setEventHistory } : {
    idLieu:number,
    NomLieu:string,
        ...
        if(eventHistory[0] != undefined){
            ...
            if(eventHistory[0].startDate != "" && eventHistory[0].endDate != ""){
                ...
                var res = zoomState.data.map((elem)=>{
                    ...
                    if(measureDate >= startDate && measureDate <= endDate){
                ...
                var res = zoomState.data.map((elem)=>{
                    ...
                    // console.log("-------------------------")
                    if(measureDate.getTime() == startDate.getTime() || addNext == true){
                        elem_tmp = {...elem, Valeur_2:elem.Valeur}
                        addNext = !addNext
                    }                
                        ...
                        <>
                            ...
                            <ResponsiveContainer debounce={200} id={'container-'+idLieu} width="100%" height="100%" className="absolute overflow-hidden top-0 left-0 w-full h-full object-cover">
                                ...
                                >
                                    ...
                                    {/* <Area connectNulls type="monotone" dataKey="Valeur_2" stroke="#E50914" strokeWidth={2} fillOpacity={1} fill="url(#colorUv_red)"  isAnimationActive={isAnimated}/> */}
