...
export default function MonitoringGraphFullScreenZoomRefine(this: any, { idLieu, NomLieu, stateNeedReload, setStateNeedReload, _startDate, _endDate, eventHistory, setEventHistory } : {
    idLieu:number,
    NomLieu:string,
    ...
    // useEffect(()=>{
    //     console.log("useeffect: zoomState.right")
    //     updateGraphic();
    // }, [zoomState.right])
    ...
    // }, [zoomState.right]); // Only run when data changes
    ...
    // }, [zoomState.refAreaRight]); // Only run when data changes
    ...
    function fetchData(){
        ...
        .then(response => {
            if (Array.isArray(response.data)) {
                ...
                //     for(var i = 0; i < response.data.length; i += Math.floor((response.data.length)/10)) {  // take every second element
    ...
    function updateGraphic(){
        ...
        var tmp_zoomStartDate = zoomState.left;
        var tmp_zoomEndDate = zoomState.right;
        var index_zoomStartDate;
        var index_zoomEndDate;

        console.log(tmp_zoomStartDate)
        if(tmp_zoomStartDate != 'undefined' && tmp_zoomStartDate != ''){
            // console.log(tmp_zoomStartDate)
            _startDate = fromDate(new Date(Date.parse(tmp_zoomStartDate)), 'CET');
            console.log(data)
            console.log(tmp_zoomStartDate)
            index_zoomStartDate = data.findIndex(elem=>elem.DateHeureMesure == tmp_zoomStartDate)
            console.log("index of right dans data: "+index_zoomStartDate)
        }
        
        if(tmp_zoomEndDate != 'undefined' && tmp_zoomEndDate != ''){
            _endDate = fromDate(new Date(Date.parse(tmp_zoomEndDate)), 'CET');
            index_zoomEndDate = data.findIndex(elem=>elem.DateHeureMesure == tmp_zoomEndDate)
            console.log("index of left dans data: "+index_zoomEndDate)
        }

      
        var tmp_sliceData = data.slice(index_zoomStartDate, index_zoomEndDate)
        // console.log("updateGraphic: tmpslice_data:")
        // console.log(tmp_sliceData)
        let tmp_arrayData = [];
        if (tmp_sliceData.length > 10){
            // console.log("tickInterval: "+ tickInterval)
            // settickInterval((response.data.length)/1000);
            for(var i = 0; i < tmp_sliceData.length; i += Math.floor((tmp_sliceData.length)/10)) {  // take every second element
                tmp_arrayData.push(tmp_sliceData[i]);
            }
        } else {
            tmp_arrayData = tmp_sliceData
        }
        // setData(tmp_arrayData);
        setZoomState({...zoomState, data: tmp_arrayData});
    ...
    function zoom(e: CategoricalChartState) {
        // zoomState.refAreaLeft && setZoomState(prevState => ({
        //     ...prevState,
                    ...
                    : 
                        <>
                            ...
                            <div className="absolute left-full -translate-x-full text-right py-3 px-4 font-mono">
                                ...
                                <div>
                                    Dernière mesure du {ZonedDateTimeToString(fromDate(new Date(data[0].DateHeureMesure), "CET"), 1)} : <b>{data[0].Valeur}{data[0].Unite}</b>
                                </div>
                            </div>
                        </>
                    }     
