...
assert.match(analysis, /hasPermission\("ALARM_ACK_ACCESS"\)/)
assert.match(analysis, /markAlarmAcknowledgedInPaginatedSensorsCache/)
assert.match(analysis, /queryClient\.invalidateQueries\(\{ queryKey: \["capteurs", "paginated"\] \}\)/)
...
assert.match(analysis, /limitTodayRange: false/)
...
const rangeMeasurements = read("src/components/monitoring-details/use-monitoring-range-measurements.ts")
assert.match(rangeMeasurements, /limitTodayRange\?: boolean/)
assert.match(rangeMeasurements, /limitTodayRange = true/)
assert.match(rangeMeasurements, /limitTodayRange && isTodayRange/)
